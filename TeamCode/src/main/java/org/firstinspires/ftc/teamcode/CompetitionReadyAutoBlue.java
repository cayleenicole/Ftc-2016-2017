 ...
 */
@Autonomous(name = "BlueAuto", group = "Cayles")
public class CompetitionReadyAutoBlue extends LinearOpMode {
    ...
    private void beaconSwitchCase(){
        ...
        switch(value){
            ...
            case 5: encoderDrive(DRIVE_SPEED,  21.25,  -21.25, 1.5, headingEncoder);
                break;
            case 6: gyroRotate(0.5, 75);
                break;
            case 7: moveToWall(0.5);
                break;
            case 8: strafeToLine(0.5,currentTime);
                break;
            case 9: currentTime = getRuntime();while(getRuntime() - currentTime < 2.0 && opModeIsActive())strafe(0.5, "left");value++;
                break;
            case 20: moveToWall(0.5);
                break;
            case 10: strafeToLine(0.5,currentTime);
                break;
            case 11: currentTime = getRuntime();while(getRuntime() - currentTime < 5.0 && opModeIsActive())strafe(0.5, "right");value++;
                break;
    ...
    private void strafeToLine(double speed, double time){
        ...
        while(getRuntime() - currentTime < 0.25) {
